class QuickPagination {
    ...
    showPage(anchor, index) {
        ...
        if (anchor.parentElement.id !== 'pagination-next-button' &&
            anchor.parentElement.id !== 'pagination-prev-button') {
            anchor.parentElement.classList.add('active');
            const pageEl = this._pagesTarget.querySelector('[data-page="' + anchor.dataset.pageAnchorTag + '"]');
            pageEl.style.display = this._pageDisplay;
            this._currentPage = parseInt(anchor.dataset.pageAnchorTag);
        }
        else {
            this._paginationElement
                .querySelector('[data-page-anchor-tag="' + index + '"]')
                .parentElement.classList.add('active');
            const pageEl = this._pagesTarget.querySelector('[data-page="' + index + '"]');
            pageEl.style.display = this._pageDisplay;
    ...
    addClickEvents() {
        this._paginationElement.querySelectorAll('a').forEach((anchor) => {
            if (parseInt(anchor.dataset.pageAnchorTag) === this._currentPage) {
                this.showPage(anchor, this._currentPage);
            }
            anchor.addEventListener('click', (e) => {
                e.preventDefault();
                if (!anchor.dataset.pageAnchorTag && this._nextPrevButtons) {
                    this.showPage(anchor, this._currentPage + parseInt(anchor.dataset.pageNumber));
                }
                else {
                    this.showPage(anchor, this._currentPage);
                }
    ...
    enableListItem() {
        if (this._nextPrevButtons === false)
            return;
        let prevButton = this._paginationElement.children[0].firstElementChild;
        let nextButton = this._paginationElement.children[0].lastElementChild;
    ...
    replaceElementsWithDots(numsBefore, numsAfter) {
        ...
        if (numsAfter.length > 0) {
            this._paginationElement.children[0].insertBefore(this.createEmptyListItem(), numsAfter[numsAfter.length - 1].parentNode);
    ...
    replaceNumbersWithDots() {
        let numsBefore = [];
        let numsAfter = [];
        this._paginationElement.querySelectorAll('[data-page-anchor-tag]').forEach((item) => {
