...
class QuickPagination {
   ...
   */
  createPageNav(): HTMLOListElement {
   ...
   */
  hideAllPages() {
    this._pagesTarget.querySelectorAll('[data-page]').forEach((page: HTMLElement) => {
  ...
  showPage(anchor: HTMLElement, index: number) {
    ...
    if (
      ...
      const pageEl: HTMLElement = this._pagesTarget.querySelector(
      ...
      const pageEl: HTMLElement = this._pagesTarget.querySelector('[data-page="' + index + '"]');
      pageEl.style.display = this._pageDisplay;
  ...
  enableListItem() {
    ...
    let prevButton: Element = this._paginationElement.children[0].firstElementChild;
    let nextButton: Element = this._paginationElement.children[0].lastElementChild;
  ...
  replaceNumbersWithDots() {
    let numsBefore = [];
    let numsAfter = [];
    this._paginationElement.querySelectorAll('[data-page-anchor-tag]').forEach((item: HTMLElement) => {
