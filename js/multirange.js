...
const
    rangeEls = document.querySelectorAll('input[type="range"][multiple]');
    ...
    setValue = ()=>{
    const
        splittedValue = range.value.split(',');
        newMinValue = Number( (splittedValue[0] - range.min) * 100 / (range.max - range.min) ),
        ...
        newMaxValue = Number( (splittedValue[1] - range.min) * 100 / (range.max - range.min) ),
